...
/**
 * Builds the compact primary-connection state shown beside the Soulseek setup
 * controls. The result always describes the last saved server observation;
 * it does not imply that unsaved form input has been tested.
 *
 * @param {object[]} dependencies
 * @param {string|null|undefined} loadError
 * @returns {{message: string, observedAt: string|null, status: string, statusClass: string, statusLabel: string}}
 */
export function buildSoulseekConnectionHealthSummary(dependencies = [], loadError = null) {
  if (loadError) {
    return {
      message: 'Harmoniarr could not refresh the saved connection status.',
      observedAt: null,
      status: 'unknown',
      statusClass: getDependencyStatusClass('unknown'),
      statusLabel: 'Unable to check',
    };
  }

  const dependency = dependencies.find((item) => item?.provider === 'slskd');
  if (!dependency) {
    return {
  ...
  return {
    message: dependency.message ?? `Soulseek is ${formatDependencyStatusLabel(dependency.status).toLowerCase()}.`,
    observedAt: dependency.details?.observedAt ?? null,
    status: dependency.status ?? 'unknown',
    statusClass: getDependencyStatusClass(dependency.status),
    statusLabel: formatDependencyStatusLabel(dependency.status),
  };
}

/**
 * Removes the primary download provider from the supporting diagnostics list.
 * Keeping the resulting list separate prevents it from competing with the
 * setup and test path above it.
 *
 * @param {object[]} dependencies
 * @returns {object[]}
 */
export function getSupportingProviderHealth(dependencies = []) {
  return dependencies.filter((dependency) => dependency?.provider !== 'slskd');
}
