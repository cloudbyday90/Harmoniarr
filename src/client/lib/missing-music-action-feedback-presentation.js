...
/**
 * Creates the one short-lived Missing Music action result. It is intentionally
 * release-keyed and bounded so asynchronous feedback stays in the current
 * decision context without becoming an in-page notification feed.
 *
 ...
 */
export function createMissingMusicActionFeedback({
  actionKey,
  message,
  phase,
  wantedReleaseId,
...
/**
 * Returns a display-safe action result only for the selected Missing Music
 * release. Routine progress and completion retain a polite `status` role;
 * errors retain `alert` so assistive technologies can announce outcomes.
 *
 * @param {ReturnType<typeof createMissingMusicActionFeedback> | null | undefined} feedback
 ...
 */
export function buildMissingMusicReleaseActionFeedback(feedback, wantedReleaseId) {
  const normalizedReleaseId = normalizeText(wantedReleaseId);
