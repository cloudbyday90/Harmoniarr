...
const ADMIN_MONITORED_SORT_COLUMNS = new Map([
  ...
  ['monitored_at', 'metadata_artist_monitoring.updated_at DESC'],
  ['monitored_at_asc', 'metadata_artist_monitoring.updated_at ASC'],
  ['last_refreshed', 'metadata_artist_monitoring.last_refreshed_at DESC NULLS LAST'],
  ['country', 'metadata_artists.country ASC NULLS LAST'],
]);

export async function listAdminMonitoredMetadataArtists({ search, sort, limit, offset }, queryable) {
  const db = resolveQueryable(queryable);
  const safeLimit = Math.max(1, Math.min(Number(limit) || 25, 100));
  const safeOffset = Math.max(0, Number(offset) || 0);
  const orderClause = ADMIN_MONITORED_SORT_COLUMNS.get(sort) ?? 'metadata_artists.name ASC';

  const conditions = ['metadata_artist_monitoring.is_monitored = TRUE'];
  const params = [];
  let paramIndex = 1;

  if (typeof search === 'string' && search.trim().length > 0) {
    conditions.push(`(metadata_artists.name ILIKE '%' || $${paramIndex} || '%' OR metadata_artists.sort_name ILIKE '%' || $${paramIndex} || '%')`);
  ...
  const countResult = await db.query(
    `SELECT COUNT(*)::integer AS total FROM metadata_artists JOIN metadata_artist_monitoring ON metadata_artist_monitoring.metadata_artist_id = metadata_artists.id WHERE ${whereClause}`,
    params,
  );
  ...
  const result = await db.query(
    `
      SELECT
        ...
        metadata_artist_monitoring.updated_at AS monitored_at,
        metadata_artist_monitoring.monitored_by_user_id,
        app_users.username AS monitored_by_username
      FROM metadata_artists
      JOIN metadata_artist_monitoring
        ON metadata_artist_monitoring.metadata_artist_id = metadata_artists.id
      LEFT JOIN app_users
        ON app_users.id = metadata_artist_monitoring.monitored_by_user_id
      WHERE ${whereClause}
  ...
  return {
    results: result.rows.map((row) => ({
      ...
      lastRefreshedAt: row.last_refreshed_at?.toISOString?.() ?? row.last_refreshed_at ?? null,
      localId: row.id,
      monitoredAt: row.monitored_at?.toISOString?.() ?? row.monitored_at ?? null,
      monitoredByUserId: row.monitored_by_user_id,
      monitoredByUsername: row.monitored_username ?? null,
