...
export function createLibraryMediaRequestPipelineService({
  ...
  async function buildPipeline({ mediaRequestId }) {
    if (!mediaRequestId) {
      return { candidates: [] };
    }
    ...
    const [executionItems, applyItems] = await Promise.all([
      db.query(
        `
          ...
          WHERE ei.import_candidate_id = ANY($1::text[])
      ...
      db.query(
        `
          ...
          WHERE ai.import_candidate_id = ANY($1::text[])
          ORDER BY or_.started_at DESC
        `,
        [candidateIds],
      ),
    ...
    for (const row of executionItems.rows) {
      const id = row.import_candidate_id;
      if (!executionByCandidate.has(id)) {
        executionByCandidate.set(id, mapRunItemRow(row));
      }
    }
    ...
    for (const row of applyItems.rows) {
      const id = row.import_candidate_id;
      if (!applyByCandidate.has(id)) {
        applyByCandidate.set(id, mapRunItemRow(row));
      }
    }

    const pipelineCandidates = candidates.map((candidate) => ({
      ...candidate,
      execution: executionByCandidate.get(candidate.id) ?? null,
      apply: applyByCandidate.get(candidate.id) ?? null,
    }));

    return { candidates: pipelineCandidates };
  }
