...
import { createHeartbeatState } from './heartbeat-state.js';
...
export function createPausableHeartbeatState({
  initialState = {},
  normalizeExtraState = () => ({}),
  ...
  return createHeartbeatState({
    ...
    normalizeExtraState: (state) => ({
      ...normalizePauseState(state),
      ...normalizeExtraState(state),
    }),
    resolveExtraStateForOutcome: ({ details, nextBaseState, previousState }) => ({
      ...resolvePauseStateForOutcome({ details, nextBaseState }),
      ...resolveExtraStateForOutcome({ details, nextBaseState, previousState }),
    }),
  });
}