...
| 2 | P1 | Stale dialog loads can change newer state. `src/client/composables/useReleaseDetail.js:50` aborts prior work, but response/error/finally updates have no request-identity guard; old finally clears current loading. | Add current-request guards, cancellation on disposal, and defined reset behavior. Resolve two requests in reverse order and prove only the latest changes data, errors, or loading. |
| 3 | P1 | Raw dialog error has no Retry action. `src/client/components/media/ReleaseDetailModal.vue:575`. | Add a contextual, announced error and Retry, preserve album identity, and withhold actions that need unavailable data. Verify failed load followed by successful retry without closing. |
| 4 | P1 | Edition display silently stops at six. `ReleaseDetailModal.vue:622` uses `allReleases.slice(0, 6)`. | Provide bounded expansion or an accessible selector for all supplied editions. Test seven-plus editions and selection beyond six. Coordinate with server pagination in item 14. |
| 5 | P1 | Save conflict protection can be omitted. `src/server/metadata/operator-artist-save-service.js:273` skips null revision; `src/server/routes/metadata-routes.js:268` defaults to null. | Require the expected revision, including explicit initial revision. Reject missing/null values; stale concurrent saves must return 409 with no partial writes. Actual lost updates were not reproduced. |
| 6 | P1 | Artist GET can queue reconciliation. `src/server/routes/metadata-routes.js:237`, `src/server/metadata/operator-artist-projection-service.js:189`, and actual module wiring. | Move automatic recovery to the durable worker; keep deliberate recovery on the existing protected POST. Prove repeated GETs create no jobs and worker recovery remains deduplicated. This is a confirmed read/mutation boundary issue, not a demonstrated exploit. |
| 7 | P1 | Track identity normalization accepts malformed values. `src/server/metadata/operator-track-override-service.js:52`, `:65`, `:73`. Review reproduced `1oops` to 1, 2.9 to 2, `99ms` to 99, and malformed recording MBIDs. | Use strict safe-integer and UUID validation. Cover fractional, suffixed, overflow, malformed, and legitimate nullable values. |
| 8 | P1 | Owned and missing tracks share the label In library. `ReleaseDetailModal.vue:784`. | Use distinct accessible text for actual ownership and hide decorative glyphs. Check the browser accessibility tree for both states. |
| 9 | P1 | Artwork loading violates batch bounds and loses usable results. `src/client/composables/useArtistDetailArtwork.js:161` submits all unresolved images while `src/server/routes/artwork-routes.js:143` limits batches to 50; `:114` couples background/thumbnail failures. `ArtistDetailView.vue:1168` also omits resolved modal artwork. | Batch within 50, preserve independently successful artwork, and forward the resolved cover. Test 50/51/100 images, one-role failure, and local-cover reuse. These code defects do not establish the cause of every screenshot placeholder. |
