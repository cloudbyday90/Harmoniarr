...
| 14 | Partially addressed | Detail eagerly loads all groups and editions. `src/server/metadata/metadata-read-service.js:175`; `metadata-repository.js:399`, `:436`. Fallback currently exposes only its first 25 editions. `src/client/composables/useArtistDetail.js:191` also caps remote groups at 100 and discards pagination metadata. | Remote artist groups now have bounded progressive pages and explicit completeness; see ARTIST_CATALOG_PAGINATION_OUTCOME.md. Remaining: local display/edition pagination and query benchmarks; preserve full server-side reconciliation. Benchmark a large catalog and test preferred editions beyond page one. Latency has not been benchmarked in this review. |
