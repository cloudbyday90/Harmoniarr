...
| 10 | Fixed | Release modal uses supported theme tokens and scoped contrast-aware state and focus colors. | See [design](ARTIST_THEME_TOKENS_DESIGN.md) and [outcome](ARTIST_THEME_TOKENS_OUTCOME.md). |
| 11 | Fixed | Release dialog has one visible title heading, a unique label reference, and meaningful names throughout loading/error/edition changes. | See [design](RELEASE_DIALOG_HEADING_DESIGN.md) and [outcome](RELEASE_DIALOG_HEADING_OUTCOME.md). |
...
| 14 | Partially addressed | Detail eagerly loads all groups and editions. `src/server/metadata/metadata-read-service.js:175`; `metadata-repository.js:399`, `:436`. Fallback currently exposes only its first 25 editions. `src/client/composables/useArtistDetail.js:191` also caps remote groups at 100 and discards pagination metadata. | Remote artist groups now have bounded progressive pages and explicit completeness; Artist Detail also uses a lightweight initial identity summary to avoid duplicate catalog assembly; see ARTIST_CATALOG_PAGINATION_OUTCOME.md. A separate local catalog page API now bounds group enrichment with real PostgreSQL plan evidence; see LOCAL_ARTIST_DISCOGRAPHY_PAGE_OUTCOME.md. The editor now separates complete operator summary/draft state from actor-scoped local display pages; see LOCAL_ARTIST_DISPLAY_STATE_OUTCOME.md. Global summary reads now use compact complete catalog identities without edition hydration; see COMPACT_OPERATOR_SUMMARY_OUTCOME.md. Remote edition continuation and validated off-page preview are now implemented; see REMOTE_RELEASE_EDITION_PAGINATION_OUTCOME.md. Remaining: representative workload measurement; preserve full server-side reconciliation. Production latency is not established. |
| 15 | Diagnostics fixed | Post-save activity, notification, and refresh failures were swallowed. `src/server/metadata/operator-artist-save-service.js:634`, `:637`, `:675`, `:684`. | Committed saves now emit safe structured follow-up diagnostics through explicit Activity and notification outcomes; see ARTIST_POST_SAVE_OBSERVABILITY_OUTCOME.md. Required artist Activity now persists in the same save transaction; see ARTIST_TRANSACTIONAL_ACTIVITY_OUTCOME.md. External notification delivery remains best effort. |
