...
- [ ] Enforce redaction rules for logs, diagnostics, exported evidence, and operator-visible payloads.
  ...
  - The current boundary redacts or sanitizes secret-bearing keys, bearer tokens, emails, and path-like values before those payloads reach the authenticated control plane, and targeted server tests now lock that behavior across the affected services.
  - Remaining work on this item is still broader than the current slice: structured request/application logs plus any exported evidence or support bundles should adopt the same redaction policy before this task is considered complete.
