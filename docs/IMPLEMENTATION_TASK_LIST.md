...
- Repo runtime policy is now explicit through `packageManager`, `engines`, `devEngines`, and `.nvmrc`, so local development and GitHub Actions share a Node 25.4 baseline with npm 12 instead of relying on whatever host toolchain happens to be installed.
