...
- The repo-level `npm run validate` contract currently passes end to end on the Node 24 LTS line, and the supported local baseline tracks Node 24.19.0 through `.nvmrc`, `engines`, `devEngines`, and the Docker builder image.
...
- Repo runtime policy is now explicit through `packageManager`, `engines`, `devEngines`, and `.nvmrc`, so local development and GitHub Actions share a Node 24 LTS baseline with npm 12 instead of relying on whatever host toolchain happens to be installed.
...
- [ ] Validate fresh install, upgrade, restore preview/apply, and rollback-aware deployment behavior.
  ...
  - Added and executed Docker-backed safe-add recovery acceptance with real mounted audio fixtures. A deliberately invalid provider-folder mapping stops the completed release before an apply run; repairing it produces one candidate-scoped run and leaves an unrelated completed download untouched. Collision and strict-quality scenarios remain review-only without a new automatic add item. The verifier respects the read-only application root by using its disposable staging bind mount and cleans all Compose resources. See `docs/MUSIC_QUEUE_SAFE_ADD_DOCKER_RECOVERY_ACCEPTANCE_DESIGN.md`.
