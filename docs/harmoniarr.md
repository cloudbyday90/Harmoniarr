...
- External URL fan-out now queues preparation independently for each target in the creation transaction. Candidate completion must belong to that request's current target; a parent's imported candidate does not prove delivery to its children. The authoritative design and validation are [External request target ownership](EXTERNAL_REQUEST_TARGET_OWNERSHIP_DESIGN.md) and [its outcome](EXTERNAL_REQUEST_TARGET_OWNERSHIP_OUTCOME.md). Connecting prepared provider metadata to discovery remains a separate release requirement.
