...
- **Chunk 3 (not started):** Push notifications — Web Push API, VAPID key management, server-side subscription store (`push_subscriptions` table), request-fulfilled notification trigger, `POST /api/v1/push/subscribe`, `DELETE /api/v1/push/subscribe`, client-side permission prompt + subscription management in Settings.
