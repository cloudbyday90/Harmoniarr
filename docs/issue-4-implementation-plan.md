...
**Step 11 — Complete:** Release Radar — "New this week from artists you monitor." Server-side scheduled job (15-min heartbeat via `metadata-refresh-heartbeat.js`) scans MusicBrainz for recent releases from all monitored artists. Scheduling policy: high-activity (12h), standard (24h), stable-catalog (7d) with 20% jitter. `metadata-refresh-service.js` paginates all release groups per artist, detects new ones vs. existing. `metadata-release-detection-service.js` records detection events. Radar query: `library-release-radar-store.js` joins `metadata_release_groups` + `metadata_artist_monitoring` + `metadata_artists`, filters by `first_release_date` window. `library-release-radar-service.js` splits into recent (past 30d, newest first) and upcoming (next 90d, soonest first) arrays. Route: `GET /api/v1/library/release-radar`. Client: `fetchReleaseRadar()` in `library-api.js`, `useReleaseRadar.js` composable, `release-radar-normalization.js` (`normalizeRadarReleaseForCard`, `getRadarWindowLabel`). `RequesterHomePanel.vue` renders horizontal scroll strip (up to 8 items: 4 recent + 4 upcoming) with `ReleaseCard` components and request flow via `ConfirmRequestModal`. Tests: 9 server (`library-release-radar-service.test.js`), 13 client (`useReleaseRadar.test.js`), 21 normalization (`release-radar-normalization.test.js`).
...
**Step 28 — Complete:** Release Radar + Coming Soon — `GET /api/v1/library/release-radar` returns two sections: `recent` (monitored-artist releases in the last 30 days, newest-first) and `upcoming` (releases in the next 90 days, soonest-first). Pure read query over existing `metadata_release_groups` / `metadata_artist_monitoring` data — no new schema, no background job. Server: `library-release-radar-store.js` (SQL store), `library-release-radar-service.js` (window computation, today-split, limit clamping), wired into `library-module.js` and `library-routes.js`. Client: `fetchReleaseRadar` in `library-api.js`, `release-radar-normalization.js` (`normalizeRadarReleaseForCard`, `getRadarWindowLabel`), `useReleaseRadar.js` composable. `ActivityReleasesView.vue` at `activity-releases` route replaces `ActivityComingSoonView` — full page with recent and upcoming sections, `ReleaseCard` grids, and `ConfirmRequestModal`. `RequesterHomePanel.vue` gains a horizontal-scroll radar strip (up to 8 items, "See all" link) above the artist grid. Test suites: `library-release-radar-service.test.js` (9 server), 3 new route tests in `library-routes.test.js`, `release-radar-normalization.test.js` (21 client), `useReleaseRadar.test.js` (13 client). 994 server / 602 client tests pass, no regressions.
...
- `src/client/lib/wanted-release-normalization.js` — `normalizeWantedReleaseForCard`, `getWantedStatusLabel`, `getWantedStatusTone`, `formatWantedTrackCounts` (Step 8)
...
**Status:** Implemented across `import-candidate-recovery-service.js`, `library-discovery-rediscovery-service.js`, `library-discovery-recovery-retry-service.js`, `library-discovery-request-store.js`, `library-discovery-request-service.js`, notification preferences, operation-run scheduling, and operator wanted surfaces. Mode A enqueue failures cascade to the next candidate. Mode B terminal transfer snapshots cascade through the same path, with `Completed, Rejected` receiving a delayed same-candidate retry first. When no scoped candidate remains, Harmoniarr schedules a delayed automatic rediscovery run for the release, increments `research_attempt_count`, preserves the rediscovery delay through discovery reconciliation, and reports rediscovery scheduling in execution reconciliation summaries. When the rediscovery budget is spent, Harmoniarr blocks the discovery request with `download_recovery_exhausted`, records durable recovery evidence, sends a cooldown/coalesced admin notification through the existing push notification path, and surfaces recovery attempts/timing/source IDs in admin wanted-release views. Operators can now retry an exhausted recovery from those wanted surfaces; the action clears exhausted recovery markers, records manual retry evidence, resets search/research attempts, and queues a fresh discovery dispatch.
