...
**Step 3 — In Progress:** Discover screen — `DiscoverView.vue` exists at `/app/discover`. Ships: MusicBrainz artist search via `useDiscoverSearch`, one-tap monitoring via `useArtistMonitoring`, "Done — go to Home" CTA when any artist has been monitored, `EmptyState` for no results, `ArtistCard` grid. Outstanding: the taste-graph traversal (pick an artist → see similar artists, then narrow the field) is not implemented — Discover is currently a pure search screen, not a graph exploration flow. The Step 4 blocker is now resolved; graph traversal implementation is next.
