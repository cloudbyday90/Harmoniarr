...
**Status:** Implemented across `import-candidate-recovery-service.js`, `library-discovery-rediscovery-service.js`, `library-discovery-request-store.js`, `library-discovery-request-service.js`, and operation-run scheduling. Mode A enqueue failures cascade to the next candidate. Mode B terminal transfer snapshots cascade through the same path, with `Completed, Rejected` receiving a delayed same-candidate retry first. When no scoped candidate remains, Harmoniarr now schedules a delayed automatic rediscovery run for the release, increments `research_attempt_count`, preserves the rediscovery delay through discovery reconciliation, and reports rediscovery scheduling in execution reconciliation summaries. Operator notification for final rediscovery exhaustion remains a follow-up item.
