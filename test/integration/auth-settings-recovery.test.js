...
import { withIntegrationApp } from '../../testing/integration/app-runtime.js';
...
suite('integration auth, settings, and recovery routes', () => {
  test('bootstrap, login, refresh, and logout work against a real temporary PostgreSQL database', async () => {
    await withIntegrationApp({
      run: async ({ client, getPoolFn, postgresSource }) => {
  ...
  test('authenticated settings reads and writes persist through the shared settings service', async () => {
    await withIntegrationApp({
      run: async ({ client, getPoolFn }) => {
  ...
  test('maintenance locks replay idempotently and surface through recovery diagnostics', async () => {
    await withIntegrationApp({
      run: async ({ client, getPoolFn }) => {
        await bootstrapAdmin(client);
