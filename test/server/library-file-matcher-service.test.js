...
test('matchLibraryFiles uses scopeMetadataReleaseId for high-confidence conventional tag matches', async (t) => {
  ...
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].confidence, 'high');
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].matchedBy, 'conventional_tags');
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].metadataReleaseId, 'release-scope');
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].metadataTrackId, 'track-scope');
  assert.deepEqual(writeLibraryFileMatch.mock.calls[0].arguments[0].evidence, {
...
test('matchLibraryFiles strips conventional title suffixes before matching', async (t) => {
  ...
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].matchStatus, 'matched');
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].evidence.matchedTitle, 'foil');
});
...
test('matchLibraryFiles keeps MusicBrainz recording ID matches ahead of conventional matching', async (t) => {
  ...
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].matchedBy, 'musicbrainz_recording_id');
  assert.equal(writeLibraryFileMatch.mock.calls[0].arguments[0].confidence, 'high');
});
