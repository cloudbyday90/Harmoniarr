...
import { createUser, fetchUsers, provisionUserManagedLibraryRoot, updateUser } from '../../src/client/lib/users-api.js';
...
test('users-api routes list, create, and update requests through the shared api client contract', async (t) => {
  ...
  assert.equal(globalThis.fetch.mock.calls[1].arguments[0], '/api/v1/users');
  assert.equal(globalThis.fetch.mock.calls[1].arguments[1].method, 'POST');
  assert.equal(globalThis.fetch.mock.calls[1].arguments[1].headers.get('X-CSRF-Token'), 'csrf-users');
  assert.equal(globalThis.fetch.mock.calls[2].arguments[0], '/api/v1/users/user-1');
  assert.equal(globalThis.fetch.mock.calls[2].arguments[1].method, 'PATCH');
  assert.equal(globalThis.fetch.mock.calls[2].arguments[1].headers.get('X-CSRF-Token'), 'csrf-users');
  assert.equal(globalThis.fetch.mock.calls[3].arguments[0], '/api/v1/users/user-1/provision-managed-library-root');
  assert.equal(globalThis.fetch.mock.calls[3].arguments[1].method, 'POST');
