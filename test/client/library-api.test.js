...
test('library-api startLibraryDiscoveryRun sends POST with CSRF', async (t) => {
  globalThis.document = { cookie: 'harmoniarr_csrf=csrf-discovery' };
  globalThis.fetch = t.mock.fn(async () => createJsonResponse());

  await startLibraryDiscoveryRun();

  assert.equal(globalThis.fetch.mock.callCount(), 1);
  assert.equal(globalThis.fetch.mock.calls[0].arguments[0], '/api/v1/library/discovery-runs');
  assert.equal(globalThis.fetch.mock.calls[0].arguments[1].method, 'POST');
  assert.equal(globalThis.fetch.mock.calls[0].arguments[1].headers.get('X-CSRF-Token'), 'csrf-discovery');
});
