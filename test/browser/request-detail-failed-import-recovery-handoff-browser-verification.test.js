...
suite('Request Detail failed-import recovery handoff browser verification', () => {
  before(async () => {
    try {
  ...
  test('admins can hand off a failed import candidate into Import Review recovery context', {
    ...
    await browserRuntime.runScenario(async ({ baseUrl, browserContext, page }) => {
      ...
      await page.getByText('Import validation failed for remote-peer. Reopen the candidate after correcting the source files.', {
        exact: true,
      }).waitFor();
      ...
      scenarioName: 'request_detail_failed_import_recovery_handoff',
    });
  });
});
